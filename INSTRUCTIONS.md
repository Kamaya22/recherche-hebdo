...
Commit all changed files (`weeks/...`, `themes/...`) with message `Day N reading — <theme> (<YYYY-Www>)` and push to the default branch. The GitHub Action `.github/workflows/send-reading.yml` detects the new `day-N.md` and emails its rendered content to Kamil. The first heading line of `day-N.md` (without the leading `# `) becomes the email subject, so make it descriptive. Push exactly once, when the reading is final — no drafts, and do NOT make a second commit, since each `day-N.md` push sends one email.
